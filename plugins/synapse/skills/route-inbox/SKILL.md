...
5. If creation returns only `clientThreadId`, immediately run the hook-provided provisional acceptance command, then continue the owner's original request. Do not wait for the permanent `threadId`; the created task reconciles it from its startup hook.
