...
2. Run helper `{"action":"prepare","project":"<selected local path>","identity":<exact get_identity JSON>}`. The helper verifies the saved project, binds exact IDs locally, and generates a credential privately in Keychain. It reuses existing healthy authorization after live validation. If ready, report its chosen project and stop.
...
6. After `ready`, report the destination. One bounded inbox delivery is attempted automatically; future local prompts wake checks. Remind the user to trust Synapse hooks if they have not yet done so. Never claim a particular message was delivered merely because enrollment succeeded.
