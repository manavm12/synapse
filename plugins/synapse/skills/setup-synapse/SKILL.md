---
name: setup-synapse
description: Enable, defer, inspect, reconnect, or disable Synapse incoming tasks on this machine (macOS or Windows). Use for Synapse first-use setup, Enable incoming tasks, or receiving setup status. Uses the installed plugin only; never asks users to install a runner.
---
...
Resolve these paths relative to this SKILL.md: `../../scripts/setup.mjs`, plus the platform launcher. On macOS/Linux, execute `sh <absolute path to ../../scripts/run-node.sh> <absolute setup script path>`. On Windows, `sh` is not reliably on `PATH` even when Git is installed; execute `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File <absolute path to ../../scripts/run-node.ps1> <absolute setup script path>` instead. Either way, pass a single compact JSON object followed by a newline on **stdin**. Use a process tool that supports stdin (or start a terminal process and send that JSON line using write_stdin); the helper accepts the newline without needing EOF. Do not use JSON or credentials as command arguments. The launcher uses `CODEX_MCP_NODE_PATH`, supplied by Codex, and verifies SQLite support. If missing, ask the user to update/restart Codex; never fall back to system Node/npm.

The helper returns safe JSON. Its `inspect` action returns local connection IDs and project paths for subsequent local actions only. Do not read the platform credential store, copy credential material, inspect private databases to extract secrets, or add OAuth tokens to commands. Only the credential hash from preparation is sent to MCP.
...
2. Run helper `{"action":"prepare","project":"<selected local path>","identity":<exact get_identity JSON>}`. The helper verifies the saved project, binds exact IDs locally, and generates a credential privately in the platform's own secure credential store (Keychain, Windows DPAPI-protected storage, or Linux Secret Service). It reuses existing healthy authorization after live validation, including legacy enrollment missing its receiving destination. If `ready`, report its chosen project and stop. Read the helper's final output after it exits, not an intermediate progress message.
3. If `reconnect_required`, explain authorization needs reconnecting and ask before revoking the old connection. On acceptance, repeat preparation with `"reconnect":true` and a fresh identity. If busy, another setup is in progress; do not run a competing attempt.
   If `unavailable`, retry status/setup after the temporary network or credential-store issue is resolved. Do not recommend destructive reconnect for an availability failure.
...
- Disable: on user request, `{"action":"disable","connection_id":"..."}` immediately removes the local receiving destination, then revokes enrollment. If network or credential-store cleanup fails, say local delivery is disabled but cleanup needs retry. Existing memory, queued work, and task bindings remain intact.
