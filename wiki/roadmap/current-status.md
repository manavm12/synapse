...
The baseline provides metadata-only parent notification, hidden task claim through child-only MCP, one worktree and persistent Codex thread per channel, dispatch fencing, one active job per channel, dead-worker recovery, race-safe process locking, project-isolated runtime paths, and a shared Codex App Server for follow-up turns.
