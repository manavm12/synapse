...
App Server-created child threads currently appear under Recents. The desktop project identity is not exposed to the CLI or App Server, and placing a worktree below the repository root does not attach the thread to the saved desktop project. The visible user message in each child is the actual task; claim and completion protocol instructions are supplied separately as developer instructions. After the last active Synapse worker finishes, Synapse stops its idle App Server so the desktop can open the child instead of reporting that it is open in another app.
