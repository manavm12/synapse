...
App Server-created child threads currently appear under Recents because the App Server project registry is separate from the desktop's saved-project registry. The CLI prints a reminder to attach them to the project manually in the Codex UI for now. The visible user message in each child is the actual task; claim and completion protocol instructions are supplied separately as developer instructions. After the last active Synapse worker finishes, Synapse stops its idle App Server so the desktop can open the child instead of reporting that it is open in another app.
